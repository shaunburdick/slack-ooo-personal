...
class Bot {
  ...
  handleMessage(message) {
    ...
    } else if (message.type === 'message' && (message.text) && (channel !== null)) {
      // Channel is a direct message
      if ((channel.is_im || channel.is_mpim) && this.config.app.respond.dm) {
        if (!this.responded.hasOwnProperty(channel.id) ||
          this.responded[channel.id] < Date.now()
        ) {
          ...
          this.responded[channel.id] = Date.now() + this.config.app.reminder * 60000;
        }
