...
class Bot {
  ...
  constructor(config) {
    ...
    this.lookup = {
      data: {},
      get: (id) => this.lookup.data[id] || null,
    };
  }
  ...
  handleMessage(message) {
    ...
    if (userName === `@${this.bot.identity.name}`) {
      ...
      if ((message.event === 'direct_message') && this.config.app.respond.dm) {
        if (!this.responded.hasOwnProperty(message.channel) ||
          this.responded[message.channel] < Date.now()
        ) {
          ...
          this.responded[message.channel] = Date.now() + this.config.app.reminder * 60000;
        }
  ...
  start() {
    this.controller.on('team_join,user_change,bot_group_join,bot_channel_join', (bot, message) => {
      if (message.user && message.user.id) {
        this.lookup.data[message.user.id] = message.user;
      } else if (message.channel && message.channel.id) {
        this.lookup.data[message.channel.id] = message.channel;
      }
    });
