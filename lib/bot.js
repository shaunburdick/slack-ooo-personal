...
class Bot {
    ...
    const message = {
      ...
      attachments: [{
        ...
        fields: [
          ...
          {
            title: 'Respond personally in Channel?',
            value: this.config.app.respond.channel ? (this.config.app.respond.channelPersonal ? 'yes' : 'no') : 'disabled',
            short: true
          },
          {
            title: 'Mirror pings',
            value: this.config.app.mirrorPings ? 'yes' : 'no',
            short: true
          },
          {
            title: 'Stealth Mode',
            value: this.config.app.stealthMode ? 'yes' : 'no',
            short: true
          }
        ]
      }]
  ...
  startAnnounceLoop () {
    ...
    if (this.config.app.announce.channels.length && this.config.app.announce.times.length && !this.config.app.stealthMode) {
  ...
  handleMessage (message) {
    ...
    let replaceChannelWith = channelName;
    let channelBackoffIdentifier = message.channel;
    if (this.config.app.respond.channelPersonal) {
      replaceChannelWith = userName;
      channelBackoffIdentifier = message.user;
    }
    ...
    if (userName === `@${this.bot.identity.name}`) {
      ...
      if ((message.event === 'direct_message') && this.config.app.respond.dm) {
        ...
        if (this.config.app.mirrorPings) {
          this.bot.say({
            text: `*${userName}* sent you a DM:\n${message.text}`,
            fallback: `${userName} sent a message in DM.`,
            channel: this.payload.self.id
          });
        }
        if (!this.config.app.stealthMode) {
        ...
        logger.info(`${userName} sent message in ${channelName}: ${message.text}`);
        if (this.config.app.mirrorPings) {
          this.bot.say({
            text: `*${userName}* pinged you in *${channelName}* with message:\n${message.text}`,
            fallback: `${userName} pinged you in ${channelName} with a message.`,
            channel: this.payload.self.id
          });
        }
        if (!this.config.app.stealthMode) {
          if ((this.responded.get(channelBackoffIdentifier) || 0) < Date.now()) {
            response = {
              attachments: [
                {
                  color: 'BEDEAD',
                  fallback: this.config.app.message,
                  pretext: this.config.app.response.channelprefix.replace('${channel}', replaceChannelWith),
                  text: this.config.app.message,
                  footer: `Slack OoO Personal ${PACKAGE.version} - ${PACKAGE.homepage}`,
                  mrkdwn_in: ['text', 'pretext']
                }
            ...
            this.bot.reply(message, response);
            logger.info(`@${this.bot.identity.name} responded to ${replaceChannelWith} in ${channelName} with: `, response);
            this.responded.set(channelBackoffIdentifier, Date.now() + this.config.app.reminder * 60000);
  ...
  stop () {
    if (this.bot) {
      ...
      if (this.config.app.stealthMode) {
        this.bot.say({
          text: 'Slack OoO Personal run in stealth mode is signing off.',
          channel: this.payload.self.id
        });
      } else {
        this.bot.say({
